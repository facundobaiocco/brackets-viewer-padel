...
export class BracketsViewer {
     ...
     * @param score
     */
    private renderScore(score: Array | number) {
        if (Array.isArray(score)) {
            return score
                .map((setScore, index) => {
                    const otherScore = score[index ^ 1]; // Invertir entre jugador 1 y 2
                    const isWinner = setScore > otherScore;
                    const className = isWinner ? "set-win" : "set-loss";
                    return `<span class="set-score ${className}">${setScore}</span>`;
                })
                .join("");
        }

        return `<span class="set-score">${score ?? "-"}</span>`;
    }
    ...
    private renderParticipant(
        ...
        containers.result.innerText = `${
            participant.score === undefined
                ? "-"
                : this.renderScore(participant.score)
        }`;
